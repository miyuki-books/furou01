...
**日付は日本時間で入れてください。**ランナーの環境変数 `TZ` は `Asia/Tokyo` に設定してあるので、
`date +%F` や `new Date()` はそのまま日本時間になります。UTCの日付を使うと1日ずれます。

**本文の冒頭にタイトルの見出し（`# ...`）を書かないでください。**
タイトルは frontmatter の `title` から出力されます。本文にも書くと見出しが二重になります
（ビルド側で落としてはいますが、書かないのが本来です）。本文の見出しは `##` から始めてください。
...
**書誌は先に確定させてから書いてください。** 邦題は思い出すものではなく、引くものです。
過去に、実在しない邦題を書いた記事が他の全チェックを通過して公開された事故が起きています。

1. 国立国会図書館サーチで書名から ISBN を引く（WebFetch。無料・認証不要）
   `https://ndlsearch.ndl.go.jp/api/opensearch?title=<書名>&cnt=5`
2. openBD で ISBN から正式書名・出版社・出版年月を確定する（WebFetch）
   `https://api.openbd.jp/v1/get?isbn=<ISBN13をカンマ区切り>`
3. **本文には openBD が返した正式書名を使う。**版違いに注意（同じ書名で複数の ISBN がある）
4. frontmatter に `books: [ISBN13, ...]` を並べる

`npm run check` が openBD に問い合わせ、**ISBN の実在と、本文の書名が書誌と一致するかを照合します。**
確認が取れない本は記事から外してください。
- タイトルは `operatingVariables.titlePattern.current` の型に従う
- **アフィリエイトリンクは `[書名](/go/<ISBN13>)` と書くだけでよい。**
  例: `[リファクタリング](/go/9784274224546)`
  実際のURLはワークフローが `npm run links` で楽天APIから取得して `state/links.json` に入れます。
  **あなたがURLを組み立てる必要はありませんし、組み立ててはいけません**（捏造の余地を作らないため）
